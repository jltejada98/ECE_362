...
void init_I2C1() {
	...
	GPIOB->AFR[0] |= 1<<(4*6) | 1<<(4*7); //Alternate Function Set

	RCC->APB1ENR |= RCC_APB1ENR_I2C1EN;
	//CR1 Config from notes:
	I2C1->CR1 &= ~I2C_CR1_PE;
...
void I2C1_start(uint8_t addr, uint32_t dir) {
    // Student code goes here
	I2C1->CR2 &= ~(I2C_CR2_SADD);//Clear bits
	I2C1->CR2 |= (addr << 1) & I2C_CR2_SADD; //Set to address
	if(dir == RD){
...
int I2C1_senddata(uint8_t* data, uint32_t size) {
	...
	I2C1->CR2 |= (size << 16) & I2C_CR2_NBYTES; //Something here
...
void write_EEPROM(uint16_t wr_addr, uint8_t data) {
    // Student code goes here
	uint8_t write_buf[3];
	write_buf[0] = wr_addr >> 8; //Select higher byte
	write_buf[1] = wr_addr; //select the lower byte
	write_buf[2] = data;
	I2C1_waitidle();
	I2C1_start(0x50,WR); //EEPROM ADDRES: 0x50
...
void set_gpio(int pin) {
	...
	write_buf[1] = 0 << pin;
	I2C1_waitidle();
	I2C1_start(0x20,WR); //MCP23008 ADDRESS: 0x40
