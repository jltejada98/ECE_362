...
void tty_init(void) {
    ...
    GPIOA->AFR[1] |= 1<<(4*2) | 1<<(4); //AFRH register
    RCC->APB2ENR |= RCC_APB2ENR_USART1EN; //Enable Clock to USART1
    USART1->CR1 &= ~(USART_CR1_UE);
    USART1->CR1 &= ~(1<<(28) | USART_CR1_M |  USART_CR1_OVER8 | USART_CR1_PCE); // 8 bits, one stop, no parity, 16x oversampling
    USART1->BRR = 0x1a1; //Baud rate divisor for 115200
    USART1->CR1 |= (USART_CR1_UE |USART_CR1_RE | USART_CR1_TE); //Enable Transmit and Recieve, and enable usart

    while(!(USART1->ISR & USART_ISR_TEACK)); //Wait for Teack
    while(!(USART1->ISR & USART_ISR_REACK)); //Wait for Reack
...
static int getchar_nonirq(void) {
	...
	while(!fifo_newline(&input_fifo)){
		...
		int character = USART1->RDR & USART_RDR_RDR;
